    ...
    @foreach($data as $row)
        <tr style="direction: ltr; text-align: right">
            ...
            <td>
                ...
                <a href="{{ url('panel/slide_show_category/' . $row->id . '/edit') }}">
                @endpermission
                    @if($row->parents)
                        {{ $row->parents }} -->
                    @endif
            ...
            <td class="text-center">
                @permission('edit.slide_show_category')
                <a class="action status" data-value="{{ $row->status == 1 ? 2 : 1 }}" data-id="{{ $row->id }}" href="{{ url('panel/slide_show_category/status') }}">
                @endpermission
                    @if($row->status=="1")
