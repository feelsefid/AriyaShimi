        ...
        <div class="row clearfix">
            <div class="col-lg-12">
                <div class="card">
                    <div class="header">
                        <h2>
                            @lang('user.role'): <strong>{{ $permission_role->name }}</strong>
                        </h2>
                    </div>
                    <div class="body">
                    {{ Form::open(['url' => url('panel/permission/role/' . $permission_role->id), 'class' => 'form-horizontal', 'id' => 'frm_permission']) }}
                        {{ method_field('put') }}
                        {{ csrf_field() }}
                        @foreach($data as $model => $vals)
                            <div class="btns" style="margin-top: 20px; padding: 0 !important;">
                                <strong>:: {{ $model }} ::</strong>
                                <hr style="margin: 5px">
