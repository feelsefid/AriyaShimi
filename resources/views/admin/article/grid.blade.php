    ...
    @foreach($data as $row)
        <tr style="direction: ltr; text-align: right">
            ...
            <td>
                ...
                <a href="{{ url('panel/article/' . $row->id . '/edit') }}">
                @endpermission
                    {{ $row->name }}
                @permission('edit.article')
            ...
            <td class="text-center">{{ $row->article_categories->name }}</td>
            <td class="text-center">{{$row->language}}</td>
            <td class="text-center">
                @permission('edit.article')
                <a class="action status" data-value="{{ $row->status == 1 ? 2 : 1 }}" data-id="{{ $row->id }}" href="{{ url('panel/article/status') }}">
                @endpermission
                    @if($row->status=="1")
